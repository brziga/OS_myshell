...
char preusmerjen_izhod[256]; char preusmerjen_vhod[256];
...
int dirchange(){
    char nov_dir[1000]; //recimo da ne bo daljsi od 1000
...
int dirlist(){
    char pot[1000]; //bi moglo bit dovolj (famous last words...)
    if(tokens[1] == NULL){
        getcwd(pot,1000);
    }
    else{
    ...
    char buf[2000]; //bi moralo bit dovolj;
...
int linklist(){
    ...
    char buf[1000]; //bi moralo bit dovolj;
    struct linux_dirent* vnos;
    int rezultat = 1; //getdents vrne stevilo prebranih bytov
        ...
        while(offset < rezultat){
            ...
            if(stat(pot_do_dat, &stat_vnos) == 0){
                if(S_ISDIR(stat_vnos.st_mode) && atoi(vnos->d_name) != 0){
                    ...
                    char buffercek[500]; //bi moglo bit vec kot dovolj ker nas zanimajo samo znaki na zacetku
                    ...
                    int pid; char ime[450]; char stanje; int ppid;
